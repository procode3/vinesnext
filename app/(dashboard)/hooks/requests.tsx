...
const httpGetWriters = async (session: any) => {
    try {
        const res = await fetch(
            "http://localhost:5000/api/users?populate=*&filters[role][name][$in]=Writer", {
            method: "GET",
            headers: {
...
async function httpGetClients(session: any) {
    try {
        ...
        const res = await fetch(
            "http://localhost:5000/api/users?populate=*&filters[role][name][$in]=Client", {
            method: "GET",
            headers: {
                "Content-Type": "application/json",
                'authorization': 'Bearer ' + session!.user.jwt
...
async function httpGetOrders(session: any) {
    try {
        const res = await fetch(
            "http://localhost:5000/api/orders?populate=*&filters[status][$in]=New&sort[0]=createdAt:desc")
        return res.status == 200 ? await res.json() : [];
    }
    catch (err) {
        console.error(err)
...
export { httpGetWriters, httpGetClients, httpCreateOrder, httpGetOrders };